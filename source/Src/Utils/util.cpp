...
vector<string> Utils::get_filelist(string foldname)
{
	...
	std::vector<std::string> extensions = {".jpg", ".JPG", ".JPEG", ".png", ".jpeg"};
    int key = 6;
    ...
    for (auto & dir_entry : std::filesystem::recursive_directory_iterator(foldname))
    {
		if (std::find(extensions.begin(), extensions.end(), dir_entry.path().extension()) != extensions.end()) {
        	flist.push_back(dir_entry.path());
		}
    }
